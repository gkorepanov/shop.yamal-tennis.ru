...
class ControllerCommonHeader extends Controller {
	protected function index() {
        ...
        $log = new Log("header.txt");
     

        $data_g = array();

        $get_level = function ($id) use ( &$get_level ){
        	$categories = $this->model_catalog_category->getCategories($id);
        	$result = array();
        	foreach ($categories as $category) {
        		$result[] = array(
					'name'     => $category['name'],
					'children' => $get_level($category['category_id']),
        ...
        //$log->write(print_r($get_level(0), true));

        $this->data['categories'] = $get_level(0);

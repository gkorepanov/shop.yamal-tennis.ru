...
class ControllerCommonSeoPro extends Controller {
	...
	public function __construct($registry) {
		...
		if (!$this->cache_data) {
			...
			$query = $this->db->query("SELECT LOWER(a.`keyword`) AS 'keyword', a.`query`, c.`parent_id` FROM" . DB_PREFIX . "`url_alias` a LEFT OUTER JOIN" . DB_PREFIX . "`category` c ON a.`query`=CONCAT('category_id=', c.`category_id`)");
			...
			$query = $this->db->query("SELECT LOWER(a.`keyword`) as 'keyword', a.`query`, m.`category_id` FROM " . DB_PREFIX . "`url_alias` a, " . DB_PREFIX . "`product_to_category` m WHERE a.`query`=CONCAT('product_id=', m.`product_id`) AND m.`main_category`=1");

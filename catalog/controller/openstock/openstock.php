...
class ControllerOpenstockOpenstock extends Controller {
        ...
        if(!$this->request->post['var'] || !$this->request->post['product_id'])
        {
            ...
            $var_info       = $this->model_openstock_openstock->getFinish($this->request->post['var'], $this->request->post['product_id']);
            ...
            if($var_info != false){
                if(empty($var_info)) {
                    ...
                    if(($var_info['stock'] > 0 /*|| $var_info['subtract'] == 0*/) && $var_info['active'] == 1)
                    {
                        ...
                        $json['nostock'] = $this->language->get('text_nostock'); //$product_info['stock_status'];
                    }
                }
            }else{
                $json['error'] = $this->language->get('text_id_missing');
        ...
        $json = array();
				//echo "DUMP OF POST: " . var_dump($this->request->post);
        ...
        if(!$this->request->post['product_id'])
        {
            ...
            $product_info   = $this->model_catalog_product->getProduct((int)$this->request->post['product_id']);
            $var_info = $this->model_openstock_openstock->getRelation($this->request->post['var'], $this->request->post['product_id'], $this->request->post['level']);
            ...
            /*if($var_info != false){
                if(empty($var_info)) {
                    ...
                    if(($var_info['stock'] > 0 ) && $var_info['active'] == 1)
                    {
                        if(!($var_info['subtract'] == 0 || $this->config->get('config_stock_display') == 0))
                        {
                            $json['success']            = $this->language->get('text_in_stock_avail');

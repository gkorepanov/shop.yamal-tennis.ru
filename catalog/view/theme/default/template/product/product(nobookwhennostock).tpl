...
$('#button-cart').bind('click', function() {
	...
	var str='';
	if (levels != 0) {
		str = str + $(".option-0").attr("id").replace("option-", "option[")+"]="+cur_options[0];
	}
	
	for(i=1; i < optionlevel; i++) {
			str = str + "&" + $(".option-"+i).attr("id").replace("option-", "option[")+"]="+cur_options[i];
	}
	str = str+"&quantity="+$(".product-info input[type=\'text\']").attr("value")+"&product_id="+$("#product-cart input[type=\'hidden\']").attr("value");
                ...
                setTimeout(function(){
                		...
                		if (data.status == "success") {
		                		...
		                		for(var i=optionlevel+1; i<=levels; i++) {
		                				...
		                				$(".option-"+i+" .optionChoice").addClass("off");
		                		}

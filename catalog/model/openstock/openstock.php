...
{    
		public function getFinish($var, $product_id){
        $sql = "
            SELECT * 
    ...
    public function getRelation($var, $product_id, $level){
    	//$this->load->library('log');
    	//$this->log->write("in function!");
        $sql = "
            SELECT * 
            FROM `" . DB_PREFIX . "product_option_relation` 
                WHERE `var` LIKE '".$var."%' 
        ...
        if($option_qry->num_rows)
        {
            ...
            $list = array();
            foreach ($option_qry->rows as $row) {
								$options = explode(":", $row["var"]);
								if (!in_array($options[$level], $list)) $list[] = $options[$level];
						}
						//echo var_dump($list);

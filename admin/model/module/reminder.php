...
class ModelModuleReminder extends Model {
    ...
    public function remindAboutOrder($order_id) {
        ...
        if ($order_info && !$order_info['order_status_id'] && $order_info['email']) {
            ...
            $html = $template->fetch('mail/reminder.tpl');
    ...
    public function getReminders($order_id) {
        $query = $this->db->query("SELECT * FROM reminder WHERE order_id = '" . (int)$order_id . "' ORDER BY datetime_remind DESC");
        
        return $query->rows;
    }

    public function checkEmailAndStatus($order_id) {
        $query = $this->db->query("SELECT email, order_status_id FROM `" . DB_PREFIX . "order` WHERE order_id = '" . (int)$order_id . "'");

        if ($query->num_rows) {
            if ($query->row['email'] != '' && !$query->row['order_status_id']) {
                return true;
            }
        }
    ...
    public function createReminderTables() {
        $this->db->query("CREATE TABLE IF NOT EXISTS `reminder` (
                              `id` int(11) NOT NULL AUTO_INCREMENT,
                              `order_id` int(11) NOT NULL,
                              `datetime_remind` datetime NOT NULL,
                              `datetime_visit` datetime NULL DEFAULT NULL,
                              `key` varchar(32) NOT NULL,
                              PRIMARY KEY (`id`)
                            ) ENGINE=MyISAM  DEFAULT CHARSET=utf8 AUTO_INCREMENT=1;");
    }

    public function createStatusWaiting() {
        $this->load->model('localisation/language');
        
        $languages = $this->model_localisation_language->getLanguages();

        foreach ($languages as $language) {
            $test = $this->db->query("SELECT * FROM `" . DB_PREFIX . "order_status` WHERE order_status_id = 0 AND language_id = ".(int)$language['language_id']);

            if (!$test->num_rows) {
                $this->db->query("INSERT IGNORE INTO `" . DB_PREFIX . "order_status` (
                                    `order_status_id`,
                                    `language_id`,
                                    `name`
                                ) VALUES (
                ...
                if ($last_insert_id) {
                    $this->db->query("UPDATE `" . DB_PREFIX . "order_status` SET order_status_id = 0 WHERE order_status_id = ".(int)$last_insert_id);
                }
            }

            $this->cache->delete('order_status.' . (int)$language['language_id']);
        }
    }

    public function getStatusWaiting() {
        $query = $this->db->query("SELECT language_id, name FROM " . DB_PREFIX . "order_status WHERE order_status_id = '0'");
        
        $values = array();

        foreach ($query->rows as $value) {
            $values[$value['language_id']] = $value['name'];
        }

        return $values;
    }

    public function editStatusWaiting($data) {
        $this->createStatusWaiting();

        foreach ($data as $language_id => $name) {
            $this->db->query("UPDATE " . DB_PREFIX . "order_status SET name = '" . $this->db->escape($name) . "' WHERE order_status_id = '0' AND language_id = '".(int)$language_id."'");
            $this->cache->delete('order_status.' . (int)$language_id);
        }
    ...
    public function reportTotalReminders() {
        $query = $this->db->query("SELECT count(*) AS c FROM reminder");
    ...
    public function reportTotalVisits() {
        $query = $this->db->query("SELECT count(*) AS c FROM reminder WHERE datetime_visit IS NOT NULL");
    ...
    public function reportTotalSuccess() {
        $query = $this->db->query("SELECT count(DISTINCT o.order_id) AS c FROM reminder p LEFT JOIN `" . DB_PREFIX . "order` o ON p.order_id = o.order_id WHERE o.order_status_id > '0'");
    ...
    public function reportLastVisits() {
        $query = $this->db->query("SELECT o.order_id, (SELECT os.name FROM " . DB_PREFIX . "order_status os WHERE os.order_status_id = o.order_status_id AND os.language_id = '" . (int)$this->config->get('config_language_id') . "') AS status, p.datetime_visit, p.datetime_remind FROM reminder p LEFT JOIN `" . DB_PREFIX . "order` o ON p.order_id = o.order_id WHERE p.datetime_visit IS NOT NULL ORDER BY p.datetime_visit DESC LIMIT 20");
